...
if size(I,3)==1 %grayscale image
    %probR = I;%psrGrayHistogram(I);
    probR = psrGrayHistogram(I);
...
for i=1:n,
nestR(i,:)=sort(randperm(254,thresholds)) + 1;% size(nest)=25X5....ie nXnd
end
...
% Get the current best(finding the fittest one in each nest)
[fmaxR,bestnestR,nestR,fitnessR]=get_best_nest(nestR,nestR,fitnessR,nd,probR, q);
[fmaxG,bestnestG,nestG,fitnessG]=get_best_nest(nestG,nestG,fitnessG,nd,probG, q);
[fmaxB,bestnestB,nestB,fitnessB]=get_best_nest(nestB,nestB,fitnessB,nd,probB, q);
end
N_iterR=0;
N_iterG=0;
N_iterB=0;
...
for iter=1:N_IterTotalG,
    % Generate new solutions (but keep the current best)
     ...
     [fmax1G,bestG,nestG,fitnessG]=get_best_nest(nestG,new_nestG,fitnessG,nd,probG, q);
    % Update the counter(after one go through all nests)
      N_iterG=N_iterG+n; 
    % Discovery and randomization
...
for iter=1:N_IterTotalB,
    % Generate new solutions (but keep the current best)
     ...
     [fmax1B,bestB,nestB,fitnessB]=get_best_nest(nestB,new_nestB,fitnessB,nd,probB, q);
    % Update the counter(after one go through all nests)
      N_iterB=N_iterB+n; 
    % Discovery and randomization
